    ...
    def update(self, masterYaml):
        ...
        for y in yamls:
            ...
            try:
                ...
                continue
            # Found - update it
            if y['kind'] in ['Deployment', 'StatefulSet', 'ConfigMap', 'Ingress', 'HorizontalPodAutoscaler']: # TODO: smarter update (all resources should be updateable)
                ...
                self._rememberResource(result['statusPatch']['alteredResources'], y)
                kubetools.logInfo(' * Skipping update %s %s...' % (y['kind'], kubetools.getYamlValue(y, '/metadata/name')))
